import type { Room, Player, GameState, Tile, Spike, Trap, TrapType } from '@app-types/index';
import { GameMode, GameStatus } from '@app-types/index';
...
class GameService {
  ...
  private gameIntervals: Map<string, ReturnType<typeof setInterval>[]> = new Map();

  // --- Room Management ---

  public createRoom(hostPlayer: Player): Room {
    const roomId = this.generateRoomId();
    const hostWithColor = { ...hostPlayer, color: PLAYER_COLORS[0], x: 1, y: 1 };
  ...
  public leaveRoom(roomId: string, playerId: string): { updatedRoom: Room | null, roomWasDeleted: boolean } {
    const room = this.rooms.get(roomId);
  ...
  public getAvailableRooms(): {id: string, gameMode: GameMode, playerCount: number}[] {
  ...
  public updatePlayerPosition(roomId: string, playerId: string, newPos: { x: number; y: number }): Room | undefined {
    const room = this.rooms.get(roomId);
    ...
    const isSprinting = player.isInfected && player.sprintUntil && now < player.sprintUntil;
    ...
    if (room.gameMode === GameMode.MAZE_RACE && room.gameState.maze?.grid[newPos.y]?.[newPos.x] === 1) {
        return;
    }
  ...
  public activateAbility(roomId: string, playerId: string): Room | undefined {
    ...
    if (room.gameMode === GameMode.INFECTION_ARENA) {
        ...
        if (player.isInfected) {
            if (!player.lastSprintTime || now - player.lastSprintTime > settings.SPRINT_COOLDOWN) {
                player.lastSprintTime = now;
                player.sprintUntil = now + settings.SPRINT_DURATION;
            }
        } else {
            if (!player.lastShieldTime || now - player.lastShieldTime > settings.SHIELD_COOLDOWN) {
  ...
  public submitGuess(roomId: string, playerId: string, guess: string): Room | undefined {
    const room = this.rooms.get(roomId);
    if (!room || room.gameState.status !== 'playing' || room.gameMode !== GameMode.SPY_AND_DECODE || room.gameState.phase !== 'guessing') return;
    const player = room.players.find(p => p.id === playerId);
    if (!player) return;
  ...
  public startGame(roomId: string, playerId: string, notifyCallback: NotifyCallback): Room | undefined {
      const room = this.rooms.get(roomId);
      if (!room || room.hostId !== playerId || room.gameState.status !== 'waiting') return;
      ...
      room.players.forEach(p => {
          p.score = 0; p.isEliminated = false; p.isIt = false; p.effects = [];
          p.isInfected = false; p.isSpy = false; p.guess = null;
          
          // Initial positions for non-maze games. Maze positions are set in initializeGameMode.
          if (room.gameMode === GameMode.TRAP_RUSH) {
            p.x = Math.floor(Math.random() * GRID_SIZE); p.y = 0;
  ...
  private runMoveLogic(room: Room, player: Player) {
    switch(room.gameMode) {
        case GameMode.TAG:
            if (player.isIt) {
                const taggedPlayer = room.players.find(p => !p.isIt && p.x === player.x && p.y === player.y);
                if (taggedPlayer) { player.isIt = false; taggedPlayer.isIt = true; }
            }
            break;
        case GameMode.TERRITORY_CONTROL:
            if (room.gameState.tiles) { room.gameState.tiles[player.y][player.x] = { claimedBy: player.id, color: player.color }; }
            break;
        case GameMode.MAZE_RACE:
            if (player.x === room.gameState.maze?.end.x && player.y === room.gameState.maze?.end.y) { this.endGame(room, player); }
            break;
        case GameMode.INFECTION_ARENA:
            if (player.isInfected) {
                const now = Date.now();
                const caughtPlayer = room.players.find(p => !p.isInfected && p.x === player.x && p.y === player.y);
                if (caughtPlayer && !(caughtPlayer.shieldUntil && now < caughtPlayer.shieldUntil)) {
                    caughtPlayer.isInfected = true;
                    if (room.players.every(p => p.isInfected)) { this.endGame(room); }
                }
            }
            break;
        case GameMode.TRAP_RUSH:
            if (player.y >= room.gameState.finishLine!) { this.endGame(room, player); return; }
            ...
            if (trap && !trap.revealed) {
                ...
                switch(trap.type) {
                    case 'freeze': player.effects.push({ type: 'frozen', expires: now + settings.FREEZE_DURATION }); break;
                    case 'slow': player.effects.push({ type: 'slow', expires: now + settings.SLOW_DURATION }); break;
                    case 'teleport': player.y = Math.max(0, player.y - settings.TELEPORT_DISTANCE); break;
                }
            }
            break;
    }
  ...
  private initializeGameMode(room: Room, notify: NotifyCallback) {
    ...
    const createInterval = (callback: () => void, delay: number) => {
        const interval = setInterval(() => {
            if (!this.rooms.has(room.id)) { clearInterval(interval); return; }
            callback();
            notify(room.id, room);
        }, delay);
    ...
    switch(room.gameMode) {
        case GameMode.TAG:
            ...
            createInterval(() => room.players.forEach(p => { if (!p.isIt) p.score += 1; }), GAME_SETTINGS.Tag.POINT_INTERVAL);
            break;
        case GameMode.TERRITORY_CONTROL:
        ...
        case GameMode.MAZE_RACE:
            ...
            room.players.forEach((p, index) => {
                ...
                if (startPos) {
                    p.x = startPos.x;
                    p.y = startPos.y;
                } else { // Fallback if no fair start points found
                    p.x = 1; p.y = 1;
            ...
            break;
        case GameMode.INFECTION_ARENA:
            room.players[Math.floor(Math.random() * room.players.length)].isInfected = true;
        ...
        case GameMode.TRAP_RUSH:
             createInterval(() => {
                 const now = Date.now();
                 room.players.forEach(p => { p.effects = p.effects?.filter(e => e.expires > now); });
             }, 500);
            break;
            ...
            createInterval(() => {
                 ...
                 if (room.gameState.timer <= 0) {
                     if (room.gameState.phase === 'signaling') {
                         ...
                         room.gameState.timer = GAME_SETTINGS[GameMode.SPY_AND_DECODE].GUESS_TIME;
        ...
        case GameMode.DODGE_THE_SPIKES:
            const spawner = setInterval(() => {
                if(!this.rooms.has(room.id)) { clearInterval(spawner); return; }
                room.gameState.spikes?.push({ id: crypto.randomUUID(), x: Math.floor(Math.random() * GRID_SIZE), y: 0 });
            }, GAME_SETTINGS[GameMode.DODGE_THE_SPIKES].SPIKE_SPAWN_RATE);
            
            const mover = setInterval(() => {
                if(!this.rooms.has(room.id)) { clearInterval(mover); return; }
                ...
                room.players.forEach(p => {
                    if (!p.isEliminated && spikes.some(s => s.x === p.x && s.y === p.y)) p.isEliminated = true;
                });
                
                const active = room.players.filter(p => !p.isEliminated);
                if (active.length <= 1 && room.players.length > 1) this.endGame(room, active[0] || null);
                else if (active.length === 0 && room.players.length > 0) this.endGame(room, null);
      ...
      if (winner) {
        ...
        switch(room.gameMode) {
            case GameMode.TERRITORY_CONTROL:
                ...
                room.gameState.tiles?.flat().forEach(tile => { if (tile.claimedBy) scores[tile.claimedBy] = (scores[tile.claimedBy] || 0) + 1; });
            ...
            case GameMode.INFECTION_ARENA:
                ...
                if (survivors.length === 0) {
                    ...
                    room.players.filter(p => p.isInfected).forEach(p => p.score += 1);
                } else {
            ...
            case GameMode.SPY_AND_DECODE:
                 ...
                 const correctGuessers = room.players.filter(p => p.guess === room.gameState.correctCodeId);
                 correctGuessers.forEach(p => p.score += 1);
                 if(spy && correctGuessers.some(p => !p.isSpy) && correctGuessers.length < room.players.length / 2) {
                    spy.score += 2;
  ...
  private createInitialGameState(gameMode: GameMode): GameState {
    ...
    switch(gameMode) {
        case GameMode.TERRITORY_CONTROL: baseState.tiles = Array(GRID_SIZE).fill(null).map(() => Array(GRID_SIZE).fill({ claimedBy: null, color: null })); break;
        case GameMode.MAZE_RACE: 
            const mazeData = generateMaze(GRID_SIZE, GRID_SIZE);
            baseState.maze = { grid: mazeData.grid, end: mazeData.end };
            break;
        case GameMode.DODGE_THE_SPIKES: baseState.spikes = []; break;
        case GameMode.TRAP_RUSH:
        ...
        case GameMode.SPY_AND_DECODE:
            ...
            baseState.codes = selectedCodes.map((value, i) => ({ id: String.fromCharCode(65 + i), value }));
            baseState.correctCodeId = baseState.codes[Math.floor(Math.random() * baseState.codes.length)].id;
  ...
  private calculateDistances(grid: number[][], startX: number, startY: number): number[][] {
    const distances = Array(grid.length).fill(null).map(() => Array(grid[0].length).fill(-1));
    ...
    while (head < queue.length) {
        ...
        for (const dir of directions) {
            ...
            if (ny >= 0 && ny < grid.length && nx >= 0 && nx < grid[0].length && grid[ny][nx] === 0 && distances[ny][nx] === -1) {
  ...
  private generateTrapMap(): (Trap | null)[][] {
      ...
      const map: (Trap | null)[][] = Array(GRID_SIZE).fill(null).map(() => Array(GRID_SIZE).fill(null));
