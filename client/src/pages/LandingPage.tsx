    ...
    <div className="min-h-screen bg-background text-text-primary overflow-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center p-4 overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-6xl mx-auto flex flex-col items-center">
          ...
          >
            <h1 className="text-7xl sm:text-8xl md:text-9xl font-black tracking-tighter mb-4 text-text-primary relative" style={{ textShadow: '0 5px 30px hsla(var(--primary-hsl), 0.2)' }}>
              <motion.span variants={{ hidden: { opacity: 0, y: 50 }, visible: { opacity: 1, y: 0, transition: { duration: 0.8, ease: "easeOut" } } }} className="block bg-gradient-to-r from-accent-secondary to-purple-400 bg-clip-text text-transparent">
                PIXEL
              </motion.span>
              <motion.span variants={{ hidden: { opacity: 0, y: 50 }, visible: { opacity: 1, y: 0, transition: { duration: 0.8, delay: 0.2, ease: "easeOut" } } }} className="block">
                ARENA
              </motion.span>
            </h1>
            ...
            >
              The ultimate multiplayer gaming experience. Battle friends in real-time
              across <span className="text-text-primary font-semibold">8 unique game modes</span>.
          ...
          >
            <Link to={user ? "/lobby" : "/login"}>
              <motion.button
                whileHover={{ scale: 1.05, boxShadow: '0 0 40px hsla(var(--primary-hsl), 0.5)' }}
                whileTap={{ scale: 0.95 }}
