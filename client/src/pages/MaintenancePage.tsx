...
const CountdownUnit: React.FC<{ value: number; label: string }> = ({ value, label }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative w-20 h-24 sm:w-28 sm:h-32 bg-surface-100/50 backdrop-blur-sm border border-border rounded-xl flex items-center justify-center overflow-hidden">
        ...
        <AnimatePresence mode="popLayout">
          <motion.span
            ...
            className="absolute text-5xl sm:text-7xl font-black text-text-primary tracking-tighter"
          >
            {paddedValue}
          </motion.span>
        </AnimatePresence>
      </div>
      <span className="mt-4 text-sm sm:text-base font-bold text-text-secondary uppercase tracking-widest">
        {label}
      </span>
    </div>
  );
...
const MaintenancePage: React.FC<MaintenancePageProps> = ({ title, message, launchDate }) => {
  ...
  return (
    <div className="min-h-screen bg-background text-text-primary flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        <div className="inline-flex w-20 h-20 overflow-hidden bg-surface-100 border border-border rounded-2xl mb-6">
            <img src={Logo} className="w-full h-full" />
        </div>
        <h1 className="text-4xl sm:text-6xl font-black tracking-tighter mb-4 text-text-primary">
            {title}
        </h1>
        ...
        <div className="flex justify-center items-start gap-3 sm:gap-6">
            ...
            <span className="text-5xl sm:text-7xl font-black text-primary/50 pt-8">:</span>
