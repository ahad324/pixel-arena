...
const EndScreen: React.FC<EndScreenProps> = ({ room, onBackToLobby }) => {
  ...
  const showScores = room.gameMode === GameMode.TAG || room.gameMode === GameMode.TERRITORY_CONTROL || room.gameMode === GameMode.SPY_AND_DECODE;
          ...
          {sortedPlayers.map((player, index) => (
            <div key={player.id} className="bg-gray-700 p-3 rounded-lg flex justify-between items-center">
              <div className="flex items-center">
                <span className="font-bold text-lg mr-4 text-gray-400">#{index + 1}</span>
                <div className="w-5 h-5 rounded-full mr-3" style={{ backgroundColor: player.color }}></div>
                <span className="font-semibold text-lg">{player.name}</span>
              </div>
              {showScores && (
                <div className="font-bold text-xl text-blue-300">{player.score} pts</div>
              )}
            </div>
          ))}
        </div>
